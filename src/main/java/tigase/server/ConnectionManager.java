 ...
 */
/**
 * 这是一个extend AbstractMessageReceiver的抽象类。
 * ﻿正如其名，这个类专注于对连接进行管理工作。
 * 如果你的组件需要通过网络直接发送或接受数据（比如c2s connection，s2s connection 或者 连接到外部第三方jabber服务），
 * 你可以把它作为基类进行扩展。它会帮你把所有和网络有关的工作都打理好（例如io，重连，socket监听和连接握手等工作）。
 * 如果你extend这个类，你需要知道数据来源于哪里：如果来源于MessageRouter，那么abstract void processPacket(Packet packet)方法会被调用; 
 * 如果来源于网络连接，那么abstract Queue processSocketData(XMPPIOService serv)方法会被调用。
 * @author lidaye
 *
 * @param <IO>
 */
...
public abstract class ConnectionManager<IO extends XMPPIOService<?>>
	...
	// 根据检测心跳包 关闭 客户端链接 的开关  自己加的 便于配置
	protected static  int XMPP_PING_OPEN=0;
	protected static final  String XMPP_PING_OPEN_KEY="xmpp-ping-open";
	
	protected static long MAX_SYSTEM_USER=10015;
	protected static final  String MAX_SYSTEM_USER_KEY="xmpp-ping-open";
...
//							|| (serv.getTotalPacketsSent() >= total_packets_limit)
	...
	public Map<String, Object> getDefaults(Map<String, Object> params) {
		...
		if ( params.get( "--" + XMPP_PING_OPEN_KEY ) != null ){
			XMPP_PING_OPEN = Integer.valueOf( (String)params.get( "--" + XMPP_PING_OPEN_KEY ) );
		}
		props.put( XMPP_PING_OPEN_KEY, XMPP_PING_OPEN);
		
		
		if ( params.get( "--" + MAX_SYSTEM_USER_KEY ) != null ){
			MAX_SYSTEM_USER = Integer.valueOf( (String)params.get( "--" + MAX_SYSTEM_USER_KEY ) );
		}
		props.put( MAX_SYSTEM_USER_KEY, MAX_SYSTEM_USER);
	...
	protected boolean writePacketToSocket(Packet p, String serviceId) {
		System.out.println("ConnectionManager:writePacketToSocket--p:>"+p.toString()+"  ==serviceid:"+serviceId);
		...
		public void run() {
			while (true) {
				try {
					...
					++watchdogRuns;
					//System.out.println(" Watchdog run  watchdogRuns ==> "+watchdogRuns);
						...
						public void check(final XMPPIOService service) {
							try {
								if ( null != service ){
									...
									long time=curr_time - lastTransfer;
									if ( time >= maxInactivityTime ){
										...
										++watchdogStopped;
										service.stop();
										System.out.println(" Watchdog stop Service By maxInactivityTime ==> "+service.getUserJid());
									} else {
										if ( time >= ( watchdogTimeout ) ){
											...
											switch ( watchdogPingType ) {
												case XMPP:
													 ...
													 if(1==XMPP_PING_OPEN){
														// long userId=0;
														 String str=service.getUserJid().toString().split("@")[0];
															
														 if(MAX_SYSTEM_USER<Long.parseLong(str)){
															 System.out.println(" Watchdog watchdogTimeout ==> "+(time) +" ==> "+service.getUserJid());
															 int xmppVersion=UserDao.getInstance().getUserXmppVersion(service.getUserJid());
															 /// System.out.println(" Watchdog timeout xmppVersion ==> "+xmppVersion);
															if(1==xmppVersion){
																writePacket=false;
																++watchdogStopped;
																service.stop();
																System.out.println(" Watchdog stop Service By watchdogTimeout ==> "+service.getUserJid());
															}else if(time>watchdogTimeout*3){
																//客户端没有发心跳包机制 链接断网了  服务试图和它建立链接 失败
																writePacket=false;
																++watchdogStopped;
																service.stop();
																System.out.println(" Watchdog stop Service By watchdogTimeout ==> "+service.getUserJid());
													 ...
													 if(writePacket){
														 boolean falg=writePacketToSocket( (IO) service, pingPacket );
														 
														// System.out.println("Watchdog send ping  "+pingPacket.getElement().toString());
													     //System.out.println("Watchdog send ping result is > "+falg);
														if (!falg){
