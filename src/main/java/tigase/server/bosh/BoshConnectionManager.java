...
public class BoshConnectionManager extends ClientConnectionManager implements BoshSessionTaskHandler {
	...
	protected enum BOSH_OPERATION_TYPE {
		...
		CREATE, REMOVE, INVALID_SID, TIMER;
	...
	protected Map<String, String> preBindSession(Map<String, String> attr) {
		...
		BoshSession bs = new BoshSession(getDefVHostItem().getDomain(),
				...
				sendNodeHostname ? getDefHostName().getDomain() : null, maxSessionWaitingPackets);
	...
	public Queue<Packet> processSocketData(XMPPIOService<Object> srv) {
		...
		while ((p = serv.getReceivedPackets().poll()) != null) {
			//System.out.println("bohsConnectionManager:processSocketData方法执行" + p.toString());
			List<Element> message = p.getElemChildren("body");
			if (null!=message) {
				for (Element messageElem : message) {
					//如果节点名为message并且含有加密字段isEncrypt为1,则进行解密
					if("message".equals(messageElem.getName())&&"1".equals(messageElem.getAttributeStaticStr("isEncrypt"))){
						Element child = messageElem.getChild("body");
						//如果body存在则解密后更换
						if(null!=child){
							//获取body内容
							String data = child.getCData();
							System.out.println("消息体"+data);
							//删除原消息体
							messageElem.removeChild(child);
							// 获取message中发送者id
							String userId = p.getAttributeStaticStr("fromLsUserId");
							Integer fromLsUserId = Integer.parseInt(userId);
							// 获取密钥
							String secretKey = RedisUtil.get(String.format(GET_SECRET_KEY, fromLsUserId));
							// 将body解密
							String newBody = AESUtil.decrypt(data, secretKey);
							if (null == newBody) {
								newBody = "出错了";
							}
	 ...
	 * 
	 * @param bs
	 *            {@link BoshSession} for which JID should be retrieved
	 * 
	...
	public BareJID getSeeOtherHostForJID(BareJID fromJID, Phase ph) {
		...
		if (log.isLoggable(Level.FINEST)) {
			log.finest("using = " + see_other_host_strategy.getClass().getCanonicalName() + "for jid = "
					+ fromJID.toString() + " got = " + ((see_other_host != null) ? see_other_host.toString() : "null")
					+ " in phase: " + ph.toString());
		}
	 ...
	 *
	 * @param out_results
	 *            collection of {@link Packet} objects to be added to queue
	 * @param bs
	 *            related {@link BoshSession}
	 */
	...
	protected void processCommand(Packet packet) {
			...
			if (jid != null) {
				if (session != null) {
					try {
						...
						if (hostJid != null) {
							Element streamErrorElement = see_other_host_strategy
									.getStreamError("urn:ietf:params:xml:ns:xmpp-streams", hostJid);
							...
							session.close();
							if (log.isLoggable(Level.FINE)) {
								log.log(Level.FINE, "{0} : {1} ({2})", new Object[] { BOSH_OPERATION_TYPE.REMOVE,
										session.getSid(), "See other host" });
							}
		...
		case CLOSE:
			if (session != null) {
				...
				session.close();
				if (log.isLoggable(Level.FINE)) {
					log.log(Level.FINE, "{0} : {1} ({2})", new Object[] { BOSH_OPERATION_TYPE.REMOVE, session.getSid(),
	...
	/**
	 * Method retrieves {@link BoshSession} related to the particular user
	 * address
	 *
	 * @param jid
	 *            address for which {@link BoshSession} should be returned
	 *
