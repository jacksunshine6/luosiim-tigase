 ...
 * @version $Rev$
 */
public class SessionManager extends AbstractMessageReceiver implements
		Configurable, SessionManagerHandler, OnlineJidsReporter {
	/** Field description */
	...
	private Map<String, XMPPProcessorIfc> processors = new ConcurrentHashMap<String, XMPPProcessorIfc>(
			32);
	private Map<String, XMPPPresenceUpdateProcessorIfc> presenceProcessors = new ConcurrentHashMap<String, XMPPPresenceUpdateProcessorIfc>();
	...
	/**
	 * A Map with connectionID as a key and an object with all the user
	 * connection data as a value
	 */
	...
	private final String REFRESH_USER_STATUS="--refresh-user-status";
	private int refreshUserStatus=0;
	...
	public boolean addOutPacket(Packet packet) {
		...
		// to make sure the connection manager for instance can report problems
		// back
	...
	public XMPPImplIfc addPlugin(String plug_id, String conc)
		...
		if (proc != null) {
			...
			if (conc != null && !conc.trim().isEmpty()) {
				...
				try {
					...
					log.log(Level.CONFIG,
							"Concurrency for plugin: {0} set to: {1}",
							new Object[] { plug_id, threadsNo });
				} catch (Exception e) {
					log.log(Level.WARNING, "Plugin " + plug_id
							+ " concurrency parsing error for: " + conc, e);
				...
				if (plug_conc.length > 1) {
					try {
						...
						log.log(Level.CONFIG,
								"Queue for plugin: {0} set to: {1} per thread",
								new Object[] { plug_id, queueSize });
					} catch (Exception e) {
						log.log(Level.WARNING, "Plugin " + plug_id
								+ " queueSize parsing error for: " + conc, e);
						queueSize = maxQueueSize / threadsNo;
					}
				}
			}

			// If there is not default processors thread pool or the processor
			// does
			// have thread pool specific settings create a separate thread pool
			// for the processor
			if ((workerThreads.get(defPluginsThreadsPool) == null)
					|| (conc != null)) {

				// Added to make sure that there will be only one thread pool
				// for plugin
				...
				if (!workerThreads.containsKey(proc.id())) {
					...
					ProcessingThreads<ProcessorWorkerThread> pt = new ProcessingThreads<ProcessorWorkerThread>(
							worker, threadsNo, queueSize, proc.id());

					workerThreads.put(proc.id(), pt);
					log.log(Level.CONFIG,
							"Created thread pool: {0}, queue per thread: {1} for plugin id: {2}",
			...
			System.out.println("Loading plugin: " + plug_id + "=" + threadsNo
					+ ":" + queueSize + " ... "
					+ (version.isEmpty() ? "" : "\t, version: " + version));
		}

		XMPPPreprocessorIfc preproc = SessionManagerConfig
				.getPreprocessor(plug_id);

		if (preproc != null) {
			preProcessors.put(plug_id, preproc);
			log.log(Level.CONFIG,
					"Added preprocessor: {0} for plugin id: {1}",
					new Object[] { preproc.getClass().getSimpleName(), plug_id });
			loaded = true;
			result = preproc;
		}

		XMPPPostprocessorIfc postproc = SessionManagerConfig
				.getPostprocessor(plug_id);

		if (postproc != null) {
			postProcessors.put(plug_id, postproc);
			log.log(Level.CONFIG,
					"Added postprocessor: {0} for plugin id: {1}",
					new Object[] { postproc.getClass().getSimpleName(), plug_id });
			loaded = true;
			result = postproc;
		}

		XMPPStopListenerIfc stoplist = SessionManagerConfig
				.getStopListener(plug_id);

		if (stoplist != null) {
			stopListeners.put(plug_id, stoplist);
			log.log(Level.CONFIG,
					"Added stopped processor: {0} for plugin id: {1}",
					new Object[] { stoplist.getClass().getSimpleName(), plug_id });
			loaded = true;
			result = stoplist;
		}

		XMPPPacketFilterIfc filterproc = SessionManagerConfig
				.getPacketFilter(plug_id);

		if (filterproc != null) {
			outFilters.put(plug_id, filterproc);
			log.log(Level.CONFIG,
					"Added packet filter: {0} for plugin id: {1}",
		...
		if (!loaded) {
			log.log(Level.WARNING,
					"No implementation found for plugin id: {0}", plug_id);
		} // end of if (!loaded)
		if (result != null) {
			...
			if (result instanceof PresenceCapabilitiesManager.PresenceCapabilitiesListener) {
				PresenceCapabilitiesManager
						.registerPresenceHandler((PresenceCapabilitiesManager.PresenceCapabilitiesListener) result);
			}
			if (result instanceof XMPPPresenceUpdateProcessorIfc) {
				presenceProcessors.put(result.id(),
						(XMPPPresenceUpdateProcessorIfc) result);
			}
		}
	...
	public void handleLogin(BareJID userId, XMPPResourceConnection conn) {
		...
		try {
			//System.out.println(conn.getConnectionId().toString());
			System.out.println("handleLogin---"+userId+"------"+conn.getConnectionId().toString());
			
			registerNewSession(userId, conn);
			if(null==userId)
				return;
			long uId=Long.parseLong(userId.toString().substring(0, userId.toString().indexOf("@")));
				conn.putSessionData("userId", uId);
			/*if(uId==10006||uId==10005||uId==10000||uId<10000)
				return;*/
				String domain=conn.getDomainAsJID().toString();
			//if(!"www.pinba.co".equals(domain)&&!"www.youjob.co".equals(domain))
				if(1==refreshUserStatus)
				//xmpp用户登录 保存登录 在线状态
				UserDao.getInstance().saveOnlineState(conn.getDomainAsJID().toString(), uId,1,conn.getConnectionId().toString());
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public void handleLogout(BareJID userId, XMPPResourceConnection conn) {
		...
		//System.out.println("SessionManager  ====>  handleLogout");
		
		
		// we need to make sure that session has no other connections as it
		// might
		...
		boolean onlyConnectionForSession = session != null
				&& (session.getActiveResourcesSize() == 0 || (session
						.getActiveResourcesSize() == 1 && session
						.getActiveResources().contains(conn)));
		...
		try {
			...
			Packet cmd = Command.CLOSE
					.getPacket(getComponentId(), conn.getConnectionId(),
							StanzaType.set, conn.nextStanzaId());
			String error = (String) conn
					.getSessionData(XMPPResourceConnection.ERROR_KEY);
			...
			log.log(Level.WARNING, "Connection ID not set for session: {0}",
					conn);
		}
	}
	...
	public void handleResourceBind(XMPPResourceConnection conn) {
		//System.out.println("SessionManager  ====>  handleResourceBind");
		
		if (!conn.isServerSession()
				&& (!"USER_STATUS".equals(conn.getSessionId()))
				&& !conn.isTmpSession()) {
			try {
				Packet user_login_cmd = Command.USER_LOGIN.getPacket(
						getComponentId(), conn.getConnectionId(),
						StanzaType.set, conn.nextStanzaId(),
						Command.DataType.submit);

				Command.addFieldValue(user_login_cmd, "user-jid", conn.getjid()
						.toString());
				...
				log.log(Level.WARNING,
						"This should not happen, check it out!, ", ex);
			}
		}
	}
	...
	public void processPacket(final Packet packet) {
		//System.out.println("sessionMag "+packet);
		if (log.isLoggable(Level.FINEST)) {
			log.log(Level.FINEST, "Received packet: {0}",
		...
		//getXMPPResourceConnection(packet)方法，返回conn。packet再由SessionManager.processPacket(packet, conn)处理。
	...
	public void removePlugin(String plug_id) {
		...
		XMPPImplIfc p = null;
		ProcessingThreads<ProcessorWorkerThread> pt = workerThreads
				.remove(plug_id);
		...
		if (p != null) {
			if (p instanceof PresenceCapabilitiesManager.PresenceCapabilitiesListener) {
				PresenceCapabilitiesManager
						.unregisterPresenceHandler((PresenceCapabilitiesManager.PresenceCapabilitiesListener) p);
			}
	...
	public void stop() {
		super.stop();
		List<String> pluginsToStop = new ArrayList<String>(
				workerThreads.keySet());
	...
	public Map<String, Object> getDefaults(Map<String, Object> params) {
		...
		if(null!=params.get(REFRESH_USER_STATUS)){
			refreshUserStatus=Integer.valueOf(params.get(REFRESH_USER_STATUS).toString()) ;
			System.out.println(getDiscoDescription()+REFRESH_USER_STATUS+"  = " + refreshUserStatus);
		}
		if(null!=params.get(UserDao.API_DB_URI_KEY)){
			UserDao.API_DB_URI_VAL=(String) params.get(UserDao.API_DB_URI_KEY);
			System.out.println(getDiscoDescription()+UserDao.API_DB_URI_KEY+"  = " + UserDao.API_DB_URI_VAL);
		}
		if(null!=params.get(UserDao.USER_DB_URI_KEY)){
			UserDao.USER_DB_URI=(String) params.get(UserDao.USER_DB_URI_KEY);
			System.out.println(getDiscoDescription()+UserDao.USER_DB_URI_KEY+"  = " + UserDao.USER_DB_URI);
		}
		if(null!=params.get(ShikuKeywordFilter.CONFIRM_OPEN_KEYWORD)){
			ShikuKeywordFilter.confirmopenkeyword=Integer.valueOf(params.get(ShikuKeywordFilter.CONFIRM_OPEN_KEYWORD).toString());
			
			if(1==ShikuKeywordFilter.confirmopenkeyword){
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						System.out.println("  getALLKeyWord end "+(endTime-startTime));
					}
				}).start();
			}
		}
		...
		props.put(STALE_CONNECTION_CLOSER_QUEUE_SIZE_KEY,
				StaleConnectionCloser.DEF_QUEUE_SIZE);
	...
	public Element getDiscoInfo(String node, JID jid, JID from) {
		if ((jid != null)
				&& (getName().equals(jid.getLocalpart()) || isLocalDomain(jid
						.toString()))) {
		...
		if (log.isLoggable(Level.FINEST)) {
			log.log(Level.FINEST,
					"Not found disco info for node: {0}, jid: {1}",
					new Object[] { node, jid });
		}
	...
	public XMPPResourceConnection getResourceConnection(JID jid) {
		...
		if (session != null) {
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST,
						"Session not null, searching session for jid: {0}", jid);
			}

			XMPPResourceConnection res = session.getResourceConnection(jid);

			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST, "Found session: {0}, for jid: {1}",
						new Object[] { res, jid });
			}
	...
	public void getStatistics(StatisticsList list) {
		...
		list.add(getName(), "Open user connections", connectionsByFrom.size(),
				Level.INFO);
		list.add(getName(), "Maximum user connections", maxUserConnections,
				Level.INFO);
		list.add(getName(), "Total user connections", totalUserConnections,
				Level.FINER);
		list.add(getName(), "Closed user connections", closedConnections,
				Level.FINER);
		list.add(getName(), "Open user sessions", sessionsByNodeId.size(),
				Level.INFO);
		list.add(getName(), "Maximum user sessions", maxUserSessions,
				Level.FINE);
		list.add(getName(), "Total user sessions", totalUserSessions,
				Level.FINER);
		list.add(getName(), "Active user connections", activeUserNumber,
				Level.FINER);
		list.add(getName(), "Authentication timouts", authTimeouts, Level.INFO);
		if (list.checkLevel(Level.INFO)) {
			int totalQueuesWait = list.getValue(getName(), "Total queues wait",
					0);
			long totalQueuesOverflow = list.getValue(getName(),
					"Total queues overflow", 0l);

			for (Map.Entry<String, ProcessingThreads<ProcessorWorkerThread>> procent : workerThreads
					.entrySet()) {
				ProcessingThreads<ProcessorWorkerThread> proc = procent
						.getValue();
				...
				if (list.checkLevel(Level.INFO,
					...
					list.add(
							...
							", Queue: " + proc.getTotalQueueSize()
									+ ", AvTime: "
									+ proc.getAverageProcessingTime()
									+ ", Runs: " + proc.getTotalRuns()
									+ ", Lost: " + proc.getDroppedPackets(),
							Level.INFO);
				}
			}
			list.add(getName(), "Total queues wait", totalQueuesWait,
					Level.INFO);
			list.add(getName(), "Total queues overflow", totalQueuesOverflow,
					Level.INFO);
		}
		if (list.checkLevel(Level.FINE)) {
			for (Map.Entry tmEntry : postTimes.entrySet()) {

				// This line is only temporary because JIndent cannot parse it
				// inside the above
				// for statement
				Map.Entry<String, long[]> entry = tmEntry;

				list.add(getName(), "Average " + tmEntry.getKey() + " on last "
						+ entry.getValue().length + " runs [ms]",
						calcAverage(entry.getValue()), Level.FINE);
			}
		}
		list.add(getName(), "Maximum user sessions today",
				maxUserSessionsDaily, Level.INFO);
		list.add(getName(), "Maximum user sessions yesterday",
				maxUserSessionsYesterday, Level.INFO);
	...
	public void setProperties(Map<String, Object> props)
		...
		if (props.get(FORCE_DETAIL_STALE_CONNECTION_CHECK) != null) {
			forceDetailStaleConnectionCheck = (Boolean) props
					.get(FORCE_DETAIL_STALE_CONNECTION_CHECK);
			log.log(Level.CONFIG,
					"forced detailed stale connection checking is set to = {0}",
					forceDetailStaleConnectionCheck);
		}
		if (props.get(STALE_CONNECTION_CLOSER_QUEUE_SIZE_KEY) != null) {
			staleConnectionCloser.setMaxQueueSize((Integer) props
					.get(STALE_CONNECTION_CLOSER_QUEUE_SIZE_KEY));
			log.log(Level.CONFIG,
					"stale connection closer queue is set to = {0}",
					staleConnectionCloser.getMaxQueueSize());
		}
		if (!staleConnectionCloser.isScheduled()) {
			addTimerTask(staleConnectionCloser,
					staleConnectionCloser.getTimeout());
		}
		...
		user_repository = (UserRepository) props
		...
		if (user_repository != null) {
			...
			for (Map.Entry<String, Object> entry : props.entrySet()) {
				if (entry.getKey().startsWith(
						RepositoryFactory.USER_REPO_PARAMS_NODE)) {

					// Split the key to configuration nodes separated with '/'
					String[] nodes = entry.getKey().split("/");

					// The plugin ID part may contain many IDs separated with
					// comma ','
					if (nodes.length > 1) {
						user_repo_params.put(nodes[1], entry.getValue()
								.toString());
					}
				}
			}
			try {
				...
				// String cls_name = (String)
				// props.get(USER_REPO_CLASS_PROP_KEY);
				String res_uri = (String) props
						.get(RepositoryFactory.USER_REPO_URL_PROP_KEY);

				user_repository = RepositoryFactory.getUserRepository(null,
						res_uri, user_repo_params);
				log.log(Level.CONFIG,
						"Initialized {0} as user repository: {1}",
						new Object[] { null, res_uri });
			} catch (Exception e) {
				log.log(Level.SEVERE, "Can't initialize user repository: ", e);
			} // end of try-catch
		}
		auth_repository = (AuthRepository) props
				.get(RepositoryFactory.SHARED_AUTH_REPO_PROP_KEY);
		if (auth_repository != null) {
			...
			for (Map.Entry<String, Object> entry : props.entrySet()) {
				if (entry.getKey().startsWith(
						RepositoryFactory.AUTH_REPO_PARAMS_NODE)) {
					...
					// comma ','
					if (nodes.length > 1) {
						auth_repo_params.put(nodes[1], entry.getValue()
								.toString());
					}
				}
			}
			try {
				String res_uri = (String) props
						.get(RepositoryFactory.AUTH_REPO_URL_PROP_KEY);

				auth_repository = RepositoryFactory.getAuthRepository(null,
						res_uri, auth_repo_params);
				log.log(Level.CONFIG,
						"Initialized {0} as auth repository: {1}",
						new Object[] { null, res_uri });
			} catch (Exception e) {
				log.log(Level.SEVERE, "Can't initialize auth repository: ", e);
			} // end of try-catch
		}
		naUserRepository = new NonAuthUserRepositoryImpl(user_repository,
				getDefHostName(), Boolean.parseBoolean((String) props
						.get(AUTO_CREATE_OFFLINE_USER_PROP_KEY)));
		synchronized (this) {
			LinkedHashMap<String, String> plugins_concurrency = new LinkedHashMap<String, String>(
					20);
			String[] plugins_conc = ((String) props
					.get(PLUGINS_CONCURRENCY_PROP_KEY)).split(",");
			...
			try {
				if (!isInitializationComplete()) {
					String sm_threads_pool = (String) props
							.get(SM_THREADS_POOL_PROP_KEY);

					if (!sm_threads_pool.equals(SM_THREADS_POOL_PROP_VAL)) {
						String[] threads_pool_params = sm_threads_pool
								.split(":");
						int def_pool_size = 100;

						if (threads_pool_params.length > 1) {
							try {
								def_pool_size = Integer
										.parseInt(threads_pool_params[1]);
							} catch (Exception e) {
								log.log(Level.WARNING,
						...
						ProcessorWorkerThread worker = new ProcessorWorkerThread();
						ProcessingThreads<ProcessorWorkerThread> pt = new ProcessingThreads<ProcessorWorkerThread>(
								worker, def_pool_size, maxQueueSize,
								defPluginsThreadsPool);

						workerThreads.put(defPluginsThreadsPool, pt);
						log.log(Level.CONFIG,
								"Created a default thread pool: {0}",
								def_pool_size);
					}
				}
				...
				log.log(Level.CONFIG, "Loaded plugins list: {0}",
				...
				for (String plug_id : plugins) {
					keys.remove(plug_id);
					log.log(Level.CONFIG,
							"Loading and configuring plugin: {0}", plug_id);

					XMPPImplIfc plugin = addPlugin(plug_id,
							plugins_concurrency.get(plug_id));

					if (plugin != null) {
						Map<String, Object> plugin_settings = getPluginSettings(
								plug_id, props);
						...
						try {
							...
							log.log(Level.SEVERE,
									"Problem initializing plugin: "
											+ plugin.id(), ex);
						}
					}
				} // end of for (String comp_id: plugins)
			} catch (Exception e) {
				log.log(Level.SEVERE, "Problem with component initialization: "
						+ getName(), e);
			}
			for (String key : keys) {
				removePlugin(key);
		...
		if (!isInitializationComplete()) {
			smResourceConnection = new SMResourceConnection(null,
	...
	protected void closeConnection(XMPPResourceConnection connection,
			JID connectionId, String userId, boolean closeOnly) {
		if (log.isLoggable(Level.FINER)) {
			log.log(Level.FINER, "Stream closed from: {0}", connectionId);
		}
		try {
			//xmpp离线 修改在线状态
			String connIdStr=null;
			if(null!=connectionId)
				connIdStr=connectionId.toString();
			//System.out.println("SessionData ---userId---"+connection.getSessionData("userId"));
			boolean refreshDB=true;
			String domain=null;
			Long uId=null;
			if(null!=connection){
				domain=connection.getDomainAsJID().toString();
				if(null==userId&&null!=connection.getjid()&&null!=connection.getjid().getBareJID())
					userId=connection.getjid().getBareJID().toString();
				if(null==connIdStr&&null!=connection.getConnectionId())
					connIdStr=connection.getConnectionId().toString();
			}
			if(null!=userId){
				 uId=Long.parseLong(userId.substring(0, userId.toString().indexOf("@")));
				/*if(uId==10006||uId==10005||uId==10000||uId<10000||uId>=new Long("3000000000"))
					refreshDB=false;*/
			}
			
			/*if(null==domain&&null!=connIdStr)
				domain=connIdStr.substring(connIdStr.indexOf("/")+1,connIdStr.indexOf("_"));
			if(null==domain&&null!=userId){
				if(userId.contains("/"))
					domain=userId.substring(userId.indexOf("@")+1,userId.indexOf("/"));
				else domain=userId.substring(userId.indexOf("@")+1,userId.length());
			}*/
			
			/*if(!"www.pinba.co".equals(domain)&&!"www.youjob.co".equals(domain))*/
			if(1==refreshUserStatus)
				UserDao.getInstance().saveOfflineTime(domain,uId,connIdStr);
			System.out.println("closeConnection ---userId---"+userId+" --connectionIsNULL---"+(null==connection));
			System.out.println("connectionId---"+connIdStr);
		} catch (Exception e) {
		...
		if (connection != null) {
			...
			// Make sure no other stuff happen on the connection while it is
			// being
			// closed. The best example is handleLogin, it happens they ares
			// called
			...
			if (log.isLoggable(Level.FINE)) {
				log.log(Level.FINE,
						"Can not find resource connection for connectionId: {0}",
						connectionId);
			}
			if (userId != null) {

				// check using userId if we can find stale
				...
				XMPPSession sessionByUserId = sessionsByNodeId.get(userJid
						.getBareJID());

				if (sessionByUserId != null) {
					connection = sessionByUserId
							.getResourceForConnectionId(connectionId);
					if (connection != null) {
						if (log.isLoggable(Level.FINEST)) {
							log.log(Level.FINEST,
			...
			// Maybe we should move this loop based check to separe thread for
			// performance reason
			// Check if our Set<JID> of not found sessions contains each of
			// available connections from each session
			...
			// code below is original loop for finding stale
			// XMPPResourceConnections
			// log.log(Level.INFO,
			// "Trying to find and remove stale XMPPResourceConnection: {0}",
			// connectionId);
			//
			...
			// if (connection != null) {
			// log.log(Level.WARNING,
			// "Found stale XMPPResourceConnection: {0}, removing...",
			// connection);
	...
	protected void closeSession(XMPPResourceConnection conn, boolean closeOnly) {
		...
		try {
			if (conn.isAuthorized()) {
				...
				if (session != null) {
					if (log.isLoggable(Level.FINE)) {
						log.log(Level.FINE, "Found parent session for: {0}",
								userJid);
					}
					// as we are closing this connection we should ensure that
					// it is removed
					...
					if (session.getActiveResourcesSize() <= 1) {
						...
						// we should check if other this is the only connection
						// on session
						// as in some cases connection can be already removed
						// from active
						// resources but there might be other connection which
						// is active
						if ((session.getActiveResourcesSize() > 0)
								&& !session.getActiveResources().contains(conn)) {
							log.log(Level.FINE,
									"Session contains connection for other "
											+ "resource than: {0}, not removing session",
									userJid);
							if (log.isLoggable(Level.FINER)) {
								StringBuilder sb = new StringBuilder(100);

								for (XMPPResourceConnection res_con : session
										.getActiveResources()) {
								...
								log.log(Level.FINER,
										...
										new Object[] {
												session.getActiveResourcesSize(),
												userJid, sb.toString() });
							}
						...
						if (session == null) {
							log.log(Level.INFO,
									"UPS can't remove, session not found in map: {0}",
									userJid);
						} else {
							if (log.isLoggable(Level.FINER)) {
								log.log(Level.FINER,
	...
	protected XMPPResourceConnection createUserSession(JID conn_id,
		...
		//System.out.println("SessionManager  ====>  createUserSession");
		if (domain != null) {
			vitem = getVHostItem(domain);
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST,
						"Setting hostname {0} for connection: {1}, VHostItem: {2}",
	...
	protected XMPPResourceConnection loginUserSession(JID conn_id,
		...
		////用户登陆Session 第一步
		//System.out.println("SessionManager  ====>  loginUserSession");
		try {
			...
			// user_repository.setData(user_id, "tokens", xmpp_sessionId,
			// conn_id.toString());
			// Authorization auth = conn.loginToken(user_id, xmpp_sessionId,
			// conn_id.toString());
			...
			log.log(Level.WARNING, "Problem logging user: " + user_id + "/"
					+ resource, ex);
		}
	...
	protected boolean processAdminsOrDomains(Packet packet) {
		if ((packet.getStanzaFrom() == null)
			...
			// The packet, probably did not went through the first state of
			// processing
			// yet.
			return false;
		}
		...
		if ((to != null) && isLocalDomain(to.toString())) {
			if (packet.getElemName() == "message") {
				...
				if (log.isLoggable(Level.FINER)) {
					log.log(Level.FINER,
							"IQ result packet addressed directly to server and not handle by any plugin: {0}",
							packet);
				}
				...
				if (log.isLoggable(Level.FINER)) {
					log.log(Level.FINER,
							"Packet for hostname, should be handled elsewhere: {0}",
							packet);
				}
			}
	...
	protected boolean processCommand(Packet pc) {
		if ((pc.getStanzaTo() == null)
				|| !(getComponentId().equals(pc.getStanzaTo()) || isLocalDomain(pc
		...
		XMPPResourceConnection connection = connectionsByFrom
				.get(iqc.getFrom());
		...
		case GETFEATURES: {
			if (iqc.getType() == StanzaType.get) {
				List<Element> features = getFeatures(connectionsByFrom.get(iqc
						.getFrom()));
		...
		case STREAM_CLOSED: {
			...
			// workerThreads.get(sessionCloseProc
			// .id());
			//
			// if (pt == null) {
			// pt = workerThreads.get(defPluginsThreadsPool);
			...
			// Replaced code above with new code below to execute STREAM_CLOSE
			// in same
			// thread as other packets from connection so next packets will know
			// there
			// is no session available after STREAM_CLOSE
			// This should not have bigger impact on performance as
			// SessionCloseProc was
			// reimplemented to speed up process of closing connections (using
			// maps instead
			// of list, etc.)
			// Updated by Artur: It does have a big impact. In most cases a DB
			// is accessed during the
			// session close processing. If this is done in the main SM thread
			// everything is slowed down.
			// If we work under load of 100 logins/logouts per second this bring
			// down the whole system.
			// see below!!
			// sessionCloseProc.process(iqc, connection, naUserRepository,
			// packetWriterQueue, plugin_config.get(sessionCloseProc.id()));
			// } catch (XMPPException ex) {
			// log.log(Level.WARNING,
			// "Exception while processing STREAM_CLOSE command", ex);
			// }
			// closeConnection(pc.getFrom(), false);
			//
			// we need to use other aproach then, as we need to remove session
			// ASAP,
			// so let's at first remove XMPPResourceConnection in this thread
			// and later add packet to
			...
			if (connection != null) {
				...
				if (session != null) {
					session.removeResourceConnection(connection);
					// now set parent session to let processors properly close
					// XMPPResourceConnection
					try {
						connection.setParentSession(session);
					} catch (TigaseStringprepException ex) {
						log.log(Level.FINE,
								"this should not happen as JID was already created once",
								ex);
					}
				}
			}

			// now we add packet to processing thread to let it close properly
			// in separate thread
			ProcessingThreads<ProcessorWorkerThread> pt = workerThreads
					.get(sessionCloseProc.id());
		...
		case STREAM_CLOSED_UPDATE: {
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST,
						"{0} processing comment, connection: {1}",
						new Object[] {
								iqc.getCommand(),
			...
			if (connection == null) {
				...
				if (stanzaFrom == null) {
					...
					// This is wrong
					log.log(Level.WARNING,
							"Stream close update without an user JID: {0}", iqc);
				} else {
					XMPPSession xs = sessionsByNodeId.get(stanzaFrom
							.getBareJID());

					if (xs == null) {
						log.log(Level.INFO,
								"Stream close for the user session which does not exist: {0}",
								iqc);
					} else {
						XMPPResourceConnection xcr = xs
								.getResourceForConnectionId(iqc.getPacketFrom());
						...
						if (xcr == null) {
							...
							if (log.isLoggable(Level.FINEST)) {
								log.log(Level.FINEST,
										"{0} removed resource connection: {1}",
		...
		case USER_STATUS:
			try {
				...
				if (prebind || isTrusted) {
					...
					boolean available = !((av != null) && av
							.equalsIgnoreCase("false"));

					JID user_jid = JID.jidInstance(Command.getFieldValue(iqc,
							"jid"));
					...
					if (prebind) {
						...
						loginUserSession(iqc.getStanzaFrom(),
								user_jid.getDomain(), user_jid.getBareJID(),
					...
					if (available) {
						...
						Element p = iqc.getElement().getChild("command")
								.getChild("presence");
						...
						if (!prebind && connection == null) {
							connection = loginUserSession(iqc.getStanzaFrom(),
									user_jid.getDomain(),
									user_jid.getBareJID(),
						...
						connection = connectionsByFrom.remove(iqc
								.getStanzaFrom());
						if (connection != null) {
							closeSession(connection, false);
							addOutPacket(iqc.okResult((String) null, 0));
						} else {
							addOutPacket(Authorization.ITEM_NOT_FOUND
									.getResponseMessage(
											iqc,
					...
					try {
						addOutPacket(Authorization.FORBIDDEN
								.getResponseMessage(iqc,
										"Only trusted entity can do it.", true));
					} catch (PacketErrorTypeException e) {
						log.warning("Packet error type when not expected: "
								+ iqc.toStringSecure());
					}
				}
			} catch (Exception e) {
				try {
					addOutPacket(Authorization.UNDEFINED_CONDITION
							.getResponseMessage(iqc,
				...
				log.log(Level.WARNING, "USER_STATUS session creation error: ",
						e);
			}
			processing_result = true;
		...
		case OTHER:
			// #2682: Commands addressed to domain should be processed by
			// sess-man
			...
			if (getComponentId().equals(iqc.getStanzaTo())
				...
				// No such command available. This prevents from an infinite
				// loop in
				// case there is no implementation to hadle such a command
				try {
					addOutPacket(Authorization.FEATURE_NOT_IMPLEMENTED
							.getResponseMessage(
									iqc,
		...
		case CLIENT_AUTH:
			if (connection != null) {
				...
				connection.putSessionData(SaslEXTERNAL.SASL_EXTERNAL_ALLOWED,
						Boolean.TRUE);
				connection.putSessionData(SaslEXTERNAL.SESSION_AUTH_JIDS_KEY,
						jids);
			}
			processing_result = true;
		...
		case STREAM_MOVED:
			if (connection != null) {
				String oldConnectionJidStr = Command.getFieldValue(pc,
						"old-conn-jid");
				JID oldConnJid = JID.jidInstanceNS(oldConnectionJidStr);

				try {

					// get old session and replace it's connection id to
					// redirect packets
					// to new connection
					XMPPResourceConnection oldConn = connectionsByFrom
							.remove(oldConnJid);
					...
					// remove current connection from list of active connections
					// as
					// this connection will be used with other already
					// authenticated connection
					sessionsByNodeId.get(oldConn.getBareJID())
							.removeResourceConnection(connection);

					Packet cmd = Command.STREAM_MOVED.getPacket(
							getComponentId(), oldConnJid, StanzaType.set,
							"moved");

					Command.addFieldValue(cmd, "cmd", "stream-moved");
					Command.addFieldValue(cmd, "new-conn-jid", oldConn
					...
					log.log(Level.SEVERE,
							"exception while replacing old connection id = "
									+ oldConnJid + " with new connection id = "
									+ pc.getPacketFrom().toString(), ex);
				}
			}
			processing_result = true;
		...
		case BROADCAST_TO_ONLINE: {
			...
			for (Element elem : pc.getElement().getChildren()) {
				if (elem.getXMLNS() == "http://tigase.org/protocol/broadcast") {
					...
					packetToBroadcast
							.setAttribute("xmlns", Packet.CLIENT_XMLNS);
				}
			}
				...
				for (XMPPSession session : sessionsByNodeId.values()) {
					...
					for (JID jid : jids) {
						...
						try {
							...
							// it is better to send by addOutPacket as in other
							// case results
							// collection could be very large!!
							addOutPacket(toSend);
						} catch (TigaseStringprepException ex) {
							log.log(Level.FINEST,
									"could not create packet for message to broadcast",
									ex);
						}
					}
				}
				...
				if (session != null) {
					...
					if (jids != null) {
						for (JID jid : jids) {
							if (log.isLoggable(Level.FINEST)) {
								log.log(Level.FINEST,
										"broadcasting packet to {0}", jid);
							}
							Element msg = packetToBroadcast.clone();
							msg.setAttribute("to", jid.toString());
							try {
								Packet toSend = Packet.packetInstance(msg);
								// it is better to send by addOutPacket as in
								// other case results
								// collection could be very large!!
								addOutPacket(toSend);
							} catch (TigaseStringprepException ex) {
	...
	protected void processPacket(Packet packet, XMPPResourceConnection conn) {
		...
		// long walkTm = 0;
		// long postTm = 0;
		// TODO: check if this is really necessary, seems to be even harmful in
		// some
		...
		if (!stop) {
			...
			if (defPacketHandler.preprocess(packet, conn, naUserRepository,
					results)) {
				packet.processedBy("filter-foward");
				if (log.isLoggable(Level.FINEST)) {
					log.log(Level.FINEST, "Packet preprocessed: {0}",
							packet.toStringSecure());
					if (results.size() > 0) {
						for (Packet p : results) {
							log.log(Level.FINEST, "Preprocess result: {0}",
									p.toStringSecure());
						}
					}
				}
		...
		if (!stop) {
			for (XMPPPreprocessorIfc preproc : preProcessors.values()) {
				stop |= preproc.preProcess(packet, conn, naUserRepository,
						results, plugin_config.get(preproc.id()));
				if (stop && log.isLoggable(Level.FINEST)) {
					log.log(Level.FINEST, "Packet blocked by: {0}, packet{1}",
							new Object[] { preproc.id(), packet });
		...
		if (!stop) {
			if (defPacketHandler.forward(packet, conn, naUserRepository,
					results)) {
				packet.processedBy("filter-foward");
				if (log.isLoggable(Level.FINEST)) {
		...
		if (!stop
				&& !packet.wasProcessed()
				&& ((packet.getStanzaTo() == null) || (!isLocalDomain(packet
						.getStanzaTo().toString())))) {
			if (defPacketHandler.canHandle(packet, conn)) {
				ProcessingThreads<ProcessorWorkerThread> pt = workerThreads
						.get(defHandlerProc.id());
		...
		if (packet.wasProcessed() || processAdminsOrDomains(packet)) {
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST, "Packet processed by: {0}", packet
						.getProcessorsIds().toString());
			}
		} else {
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST, "Packet not processed: {0}",
						packet.toStringSecure());
			}
			...
			if (stop
					|| ((conn == null) && (packet.getStanzaFrom() != null)
							...
							&& !packet.getStanzaTo().equals(getComponentId()) && ((packet
							.getElemName() == Iq.ELEM_NAME) || (packet
							.getElemName() == Message.ELEM_NAME)))) {
				try {
					error = Authorization.SERVICE_UNAVAILABLE
							.getResponseMessage(packet,
									"Service not available.", true);
				} catch (PacketErrorTypeException e) {
					log.log(Level.FINE,
							"Service not available. Packet is error type already: {0}",
							packet.toStringSecure());
				}
			} else {
			...
			if (error != null) {
				...
				try {
					...
					log.log(Level.WARNING,
							"Error packet to the SM''s own session: {0}", error);
				}
			}
		} // end of else
	...
	protected void processPresenceUpdate(XMPPSession session, Element packet) {
		try {
			...
			for (XMPPResourceConnection conn : session.getActiveResources()) {
				for (XMPPPresenceUpdateProcessorIfc proc : presenceProcessors
						.values()) {
					try {
						proc.presenceUpdate(conn, presence, packetWriterQueue);
					} catch (NotAuthorizedException ex) {
						log.log(Level.SEVERE,
								"exception processing presence update for "
			...
			// should not happen
			log.log(Level.SEVERE,
					"exception processing presence update for session = {0}"
							+ " and packet = {1}", new Object[] { session,
							packet });
		}
	}

	protected void registerNewSession(BareJID userId,
		...
		synchronized (conn) {
			...
			if (session == null) {
				...
				++totalUserSessions;
				if (log.isLoggable(Level.FINEST)) {
					log.log(Level.FINEST, "Created new XMPPSession for: {0}",
							userId);
				}
			} else {

				// Check all other connections whether they are still alive....
				List<XMPPResourceConnection> connections = session
						.getActiveResources();
				...
				if (connections != null) {
					for (XMPPResourceConnection connection : connections) {
						if (connection != conn) {
							if (log.isLoggable(Level.FINEST)) {
								log.log(Level.FINEST,
										"Checking connection: {0}", connection);
							}
							try {
								Packet command = Command.CHECK_USER_CONNECTION
										.getPacket(getComponentId(), connection
												.getConnectionId(),
												StanzaType.get, UUID
														.randomUUID()
														.toString());

								Command.addFieldValue(command, "user-jid",
										userId.toString());
								addOutPacketWithTimeout(command,
										connectionCheckCommandHandler, 30l,
										TimeUnit.SECONDS);
							} catch (NoConnectionIdException ex) {

								// This actually should not happen... might be a
								// bug:
								log.log(Level.WARNING,
										"This should not happen, check it out!, ",
										ex);
							}
						}
					}
			...
			try {
				...
				if ((!"USER_STATUS".equals(conn.getSessionId()))
						&& !conn.isServerSession() && !conn.isTmpSession()) {
					try {
						Packet user_login_cmd = Command.USER_LOGIN.getPacket(
								getComponentId(), conn.getConnectionId(),
								StanzaType.set, conn.nextStanzaId(),
						...
						Command.addFieldValue(user_login_cmd, "user-jid",
								userId.toString());
						addOutPacket(user_login_cmd);
					} catch (NoConnectionIdException ex) {
				...
				log.log(Level.INFO,
						"Stringprep problem for resource connection: {0}", conn);
	...
	protected boolean isBrokenPacket(Packet p) {
		...
		// processing loop These are most likely packets generated inside the SM
		// to
		// other users who are offline, like presence updates.
		if (getComponentId().equals(p.getPacketFrom())
				&& (p.getPacketTo() == null)) {
			return false;
		}
		if (p.getFrom() == null) {

			// This is actually a broken packet and we can't even return an
			// error
		...
		if (!p.getFrom().equals(p.getStanzaFrom())
			...
			// this is special case in which we know and expect that there will
			// be
			...
			if ((p.getElemName() == Iq.ELEM_NAME)
					|| (p.getElemName() == Message.ELEM_NAME)) {
				try {
					Packet error = Authorization.SERVICE_UNAVAILABLE
							.getResponseMessage(p, "Service not available.",
									true);

					error.setPacketTo(p.getFrom());
					fastAddOutPacket(error);
				} catch (PacketErrorTypeException e) {
					log.log(Level.FINE, "Packet is error type already: {0}",
							p.toStringSecure());
				}
			}
		...
		for (XMPPProcessorIfc proc_t : processors.values()) {
			...
			if (result == Authorization.AUTHORIZED) {
				...
				if (result != null) {
					...
					// TODO: A plugin returned an error, the packet should be
					// bounced back
	...
	public synchronized void everyMinute() {
		...
		for (BareJID bareJID : sessionsByNodeId.keySet()) {
			if (!bareJID.toString().startsWith("sess-man")) {
				for (XMPPResourceConnection xMPPResourceConnection : sessionsByNodeId
						.get(bareJID).getActiveResources()) {
					if (System.currentTimeMillis()
							- xMPPResourceConnection.getLastAccessed() < 5 * 60 * 1000) {
						count++;
					}
				}
			}
		...
		if (now.get(Calendar.YEAR) != lastDailyStatsReset.get(Calendar.YEAR)
				|| now.get(Calendar.DAY_OF_YEAR) != lastDailyStatsReset
		...
		for (Map.Entry<String, Object> entry : props.entrySet()) {
			if (entry.getKey().startsWith(PLUGINS_CONF_PROP_KEY)) {
				...
				if (nodes.length == 2) {
					...
					log.log(Level.CONFIG,
							"Adding a common plugins option: {0} = {1}",
			...
			if (entry.getKey().startsWith(PLUGINS_CONF_PROP_KEY)) {
				...
				if (nodes.length > 2) {
					...
					if (Arrays.binarySearch(ids, plug_id) >= 0) {
						...
						log.log(Level.CONFIG,
								"Adding a specific plugins option [{0}]: {1} = {2}",
								new Object[] { plug_id, nodes[1],
										entry.getValue() });
					}
				}
			}
	...
	private class AuthenticationTimer extends tigase.util.TimerTask {
			...
			if (conn != null) {
				synchronized (conn) {
					if (!conn.isAuthorized()) {
						...
						++authTimeouts;
						log.log(Level.FINE,
								"Authentication timeout expired, closing connection: {0}",
								connId);
						fastAddOutPacket(Command.CLOSE.getPacket(
								getComponentId(), connId, StanzaType.set,
								conn.nextStanzaId()));
					}
				}
			}
	...
	private class ConnectionCheckCommandHandler implements
		...
		public void responseReceived(Packet packet, Packet response) {
			if (response.getType() == StanzaType.error) {
				...
				// The connection is not longer active, closing the user session
				// here.
	...
	private class DefaultHandlerProc extends XMPPProcessor implements
			XMPPProcessorIfc {
		@Override
		...
		public void process(Packet packet, XMPPResourceConnection session,
			...
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST,
						"Executing default packet handler for: {0}", packet);
			}
	...
	private class ProcessorWorkerThread extends WorkerThread {
		...
		public void process(QueueItem item) {
			...
			try {
			    //由授权的 processor 处理 packet------> MessageAmp.process()
				processor.process(item.getPacket(), item.getConn(),
						naUserRepository, local_results,
				...
				log.log(Level.INFO, "Already error packet, ignoring: {0}", item
						.getPacket().toStringSecure());
	...
	private class SessionCloseProc extends XMPPProcessor implements
			XMPPProcessorIfc {
		@Override
		public int concurrentQueuesNo() {
			return 4;
		...
		public void process(Packet packet, XMPPResourceConnection session,
				...
				Map<String, Object> settings) throws XMPPException {
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST, "Executing connection close for: {0}",
						packet);
			}
	...
	private class SessionOpenProc extends XMPPProcessor implements
			XMPPProcessorIfc {
		@Override
		public int concurrentQueuesNo() {
			return 4;
		...
		public void process(Packet packet, XMPPResourceConnection session,
			...
			if (conn == null) {
				if (log.isLoggable(Level.FINER)) {
					log.log(Level.FINER, "Adding resource connection for: {0}",
							packet.getFrom());
				}

				final String hostname = Command.getFieldValue(packet,
						"hostname");
				...
				try {
					...
					log.log(Level.WARNING,
							"Incrrect hostname, did not pass stringprep processing: {0}",
							hostname);

					return;
				}
				addTimerTask(new AuthenticationTimer(packet.getFrom()),
						authTimeout, TimeUnit.SECONDS);
			} else {
				if (log.isLoggable(Level.FINEST)) {
					log.log(Level.FINEST,
							"Stream opened for existing session, authorized: {0}",
							conn.isAuthorized());
				}
			} // end of else
			...
			if (log.isLoggable(Level.FINEST)) {
				log.log(Level.FINEST,
						"Setting session-id {0} for connection: {1}",
	...
	private class StaleConnectionCloser extends tigase.util.TimerTask {
		...
		public void closeConnections() {
			...
			log.log(Level.INFO,
					"Trying to find and remove stale XMPPResourceConnections");
			...
			for (XMPPSession session : sessionsByNodeId.values()) {
				...
				for (XMPPResourceConnection connection : connections) {
					try {
						...
						log.log(Level.FINEST,
								"found connection without proper connection id = {0}",
								connection.toString());
					}
				}
	...
	@Override
	public void handleDomainChange(final String domain,
