...
public class ClientConnectionManager extends ConnectionManager<XMPPIOService<Object>> {
	...
	// private final Map<String, XMPPProcessorIfc> processors = new
	...
	public void processPacket(final Packet packet) {
		// System.out.println("ClientConnectionManager-processPacket->" +
		// packet.toString());
		...
		if (packet.isCommand() && (packet.getCommand() != Command.OTHER)) {
			...
			boolean writePacketToSocket = true;
			// 如果是发往web端 ,则packet.getPacketTo则为w2s
			if (packet.getTo().getLocalpart().startsWith("ws2s")) {
				// 将isEncrypt设为0
				Element messageElem = packet.getElement();
				messageElem.setAttribute("isEncrypt", "0");
				// 直接写入
				//System.out.println("发往web端:" + messageElem.toString());
				log.info("=====日 志=====:"+"发往web端,内容:" + messageElem.toString());
				System.out.println("写入前--北京时间  :"+System.currentTimeMillis());
				System.out.println("writePacketToSocket*ws2s*内容:"+packet.toString());
				writePacketToSocket = writePacketToSocket(packet);
				System.out.println("写入后--北京时间  :"+System.currentTimeMillis());
			} else {
					//判断是否有<isEncrypt>节点
					Element messageElem = packet.getElement();
					if("message".equals(packet.getElemName())&&"2".equals(messageElem.getChildStaticStr("encrypt"))){
						String body = packet.getElement().getChildCData(Message.MESSAGE_BODY_PATH);
						// 获取消息体中的接收者id
						String jBody = body.replaceAll("=&quot;", "\"").replaceAll("&quot;", "\"");
						JSONObject bodyObj = null;
						try {
							bodyObj = JSON.parseObject(body);
						} catch (Exception e) {
							log.severe("=====错 误 日 志=====:json解析失败" + "body内容:" + body);
							log.severe("=====错 误 日 志=====:json解析失败" + "packet内容:" + packet.toString());
							e.printStackTrace();
						}
						// 如果是群组聊天则,将body中添加toUserId
						if (StanzaType.groupchat == packet.getType()) {
							String toId = getToUserId(packet.getStanzaTo());
							bodyObj.put("toUserId", toId);
							body = bodyObj.toString();
						}
						Integer toUserId = bodyObj.getInteger("toUserId");
						// 获取toUserId 密钥
						String secretKey = RedisUtil.get(String.format(GET_SECRET_KEY, toUserId));
						//Element messageElem = packet.getElement();
						Element bodyElem = messageElem.getChildStaticStr("body");
						Element encrypt = messageElem.getChildStaticStr("encrypt");
						if (bodyElem != null) {
							messageElem.removeChild(bodyElem);
							messageElem.removeChild(encrypt);
						}
						// 将body加密
						String newBody = AESUtil.encrypt(body, secretKey);
						if (null == newBody) {
							newBody = body;
							log.severe("=====错 误 日 志=====:通过接收者秘钥加密失败"+"body : "+body+"秘钥:"+secretKey);
							Element testElem = new Element("body", newBody);
							Element isEncrypt = new Element("encrypt", "0");
							messageElem.addChild(testElem);
							messageElem.addChild(isEncrypt);
							messageElem.setAttribute("isEncrypt", "0");
							//messageElem.setAttribute("isEncrypt", "0");
							log.severe("=====错 误 日 志=====:通过接收者秘钥加密失败后将isEncryp属性设置 为 0");
							
						} else {
							Element testElem = new Element("body", newBody);
							Element isEncrypt = new Element("encrypt", "3");
							isEncrypt.setAttribute("xmlns", "letschat:xmpp:encrypt");
							messageElem.addChild(testElem);
							messageElem.addChild(isEncrypt);
							messageElem.setAttribute("isEncrypt", "3");
						}
					}
				// 判断是否旧包,含有属性isencrypt
				if ("message".equals(packet.getElemName())&&"2".equals(packet.getAttributeStaticStr("isEncrypt"))) {
					String body = packet.getElement().getChildCData(Message.MESSAGE_BODY_PATH);
					// 获取消息体中的接收者id
					String jBody = body.replaceAll("=&quot;", "\"").replaceAll("&quot;", "\"");
					JSONObject bodyObj = null;
				...
				System.out.println("写入前--北京时间  :"+System.currentTimeMillis());
				System.out.println("writePacketToSocket*******************内容:"+packet.toString());
				writePacketToSocket = writePacketToSocket(packet);
				System.out.println("写入后--北京时间  :"+System.currentTimeMillis());
			
		}
			if (!writePacketToSocket) {
				// if (!writePacketToSocket(packet)) {
				log.severe("=====错 误 日 志=====:理论上这里不执行:除非报错"+"packet: "+packet);
	...
	/**
	 * 从socketData中读取packet并写入队列
	 */
	@Override
	public Queue<Packet> processSocketData(XMPPIOService<Object> serv) {

		// String id = getUniqueId(serv);
		ConnectionType type = serv.connectionType();
		JID dataReceiver = serv.getDataReceiver();
		...
		while ((p = serv.getReceivedPackets().poll()) != null) {
			...
			System.out.println("从socket中读取*******************内容:"+p.toString());
			...
			// If client is sending packet with 'from' attribute set then
			// packets
			// are being duplicated in clustered environment, so best it would
			// be
			// to remove 'from' attribute as it will be set later during
			// processing
			// by SessionManager
			...
			// 如果是移动端发来的信息
			if (id.toString().startsWith("c2s")&&"message".equals(p.getElemName())) {
				Element messageElem = p.getElement();
				String userId=null;
				Integer fromLsUserId=null;
				
				//先判断是否有<encrypt>节点,<encrypt xmlns="letschat:xmpp:encrypt" from=10000078/>1</encrypt>
				Element encrypt = messageElem.getChildStaticStr("encrypt");
				if(null!=encrypt){
					userId = encrypt.getAttributeStaticStr("from");
					fromLsUserId = Integer.parseInt(userId);
//					Element fromElement = messageElem.getChildStaticStr("fromLsUserId");
//					if(null!=fromElement){
//						userId=fromElement.getCData();
//						fromLsUserId = Integer.parseInt(userId);
//					}
					// 获取密钥
					String secretKey = RedisUtil.get(String.format(GET_SECRET_KEY, userId));
					if(null==secretKey){
						log.severe("=====错 误 日 志=====:" +"获取发送者密钥为null"+"userId："+userId);
					}
					String body = messageElem.getChildCData(Message.MESSAGE_BODY_PATH);
					Element bodyElem = messageElem.getChildStaticStr("body");
					if (bodyElem != null) {
						messageElem.removeChild(bodyElem);
						messageElem.removeChild(encrypt);
					...
					// 解密代码
					String newBody = AESUtil.decrypt(body, secretKey);
					// 如果解密失败,则修改属性为0
					if (null == newBody) {
						log.severe("=====错 误 日 志=====:" + " 解密失败了body : " + body + " 密钥为 : " + secretKey);
						log.severe("=====错 误 日 志=====:" + "解密失败的packet内容 : " + p.toString());
						//如果服务端解密失败,则直接返回,不将消息写入队列
						return null;
					} else {
						// 解密成功则修改属性为2
						Element testElem = new Element("body", newBody);
						messageElem.addChild(testElem);
						Element isEncrypt = new Element("encrypt", "2");
				...
				// 如果是message,并且消息加密则进行解密相关isEncrypt:0为不加密,1为加密,2为后台已经解密,3为已经重新加密
				if ("1".equals(p.getAttributeStaticStr("isEncrypt"))) {
					// 获取message中发送者id
					userId = p.getAttributeStaticStr("fromLsUserId");
					fromLsUserId = Integer.parseInt(userId);
					// 获取密钥
					String secretKey = RedisUtil.get(String.format(GET_SECRET_KEY, userId));
					if(null==secretKey){
					...
					String body = p.getElement().getChildCData(Message.MESSAGE_BODY_PATH);
					Element bodyElem = messageElem.getChildStaticStr("body");
					if (bodyElem != null) {
						messageElem.removeChild(bodyElem);
					}
...
//						
						...
						Element testElem = new Element("body", newBody);
						messageElem.addChild(testElem);
						messageElem.setAttribute("isEncrypt", "2");
					}
				}
			}
	...
	public boolean processUndeliveredPacket(Packet packet, String errorMessage) {
		try {
			...
			// we should not send errors for presences as Presence module does
			// not
			...
			if (packet.getElemName() == Message.ELEM_NAME) {
				// we should mark this message packet so that SM will know that
				// it is
	 ...
	 * {@link SeeOtherHostIfc}
	 *
	 * @param see_other_host_class
	 *            class of {@link SeeOtherHostIfc} implementation
	 *
	...
	protected String prepareStreamError(XMPPIOService<Object> serv, String errorName, String hostname) {
		return "<?xml version='1.0'?><stream:stream" + " xmlns='" + XMLNS + "'"
				...
				+ "<stream:error>" + "<" + errorName + " xmlns='urn:ietf:params:xml:ns:xmpp-streams'/>"
	...
	protected String prepareSeeOtherHost(XMPPIOService<Object> serv, String hostname, BareJID see_other_host) {
		return "<stream:stream" + " xmlns='" + XMLNS + "'" + " xmlns:stream='http://etherx.jabber.org/streams'"
				...
				+ " version='1.0' xml:lang='en'>" + see_other_host_strategy
						.getStreamError("urn:ietf:params:xml:ns:xmpp-streams", see_other_host).toString()
				+ "</stream:stream>";
	}
	...
	private Integer getUserId(JID jid) {
		String strUserId = jid.toString();
		int index = strUserId.indexOf("/");
		strUserId = strUserId.substring(index + 1, strUserId.length());
		return Integer.parseInt(strUserId);
	}

	private String getToUserId(JID jid) {
		String strUserId = jid.toString();
		int index = strUserId.indexOf("@");
